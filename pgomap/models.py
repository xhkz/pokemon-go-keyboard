...
db = SqliteDatabase(os.path.join(os.path.dirname(os.path.realpath(__file__)), '..', 'etc', 'pgomap.db'))
...
def parse_map(map_dict):
    ...
    if pokemons:
        logger.info("Upserting {} pokemon".format(len(pokemons)))
        bulk_upsert(Pokemon, pokemons)

    if pokestops:
        logger.info("Upserting {} pokestops".format(len(pokestops)))
        bulk_upsert(Pokestop, pokestops)

    if gyms:
        logger.info("Upserting {} gyms".format(len(gyms)))
...
def bulk_upsert(cls, data):
    ...
    while i < num_rows:
        logger.debug("Inserting items {} to {}".format(i, min(i + step, num_rows)))
